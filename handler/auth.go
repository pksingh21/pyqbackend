...
func SignUp(c *fiber.Ctx) error {
	...
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"error": "Failed to hash password", "message": err.Error()})
	}
	...
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"error": "Failed to generate JWT token", "message": err.Error()})
	}
...
func JWTMiddleware() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		if err != nil {
			return c.Status(fiber.StatusUnauthorized).JSON(fiber.Map{"error": "Invalid token", "message": err.Error()})
		}
