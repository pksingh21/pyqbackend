...
const createTestQuestionStatus = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (test && question) {
    ...
    res.status(201).json({
      message: 'Test question status created',
      testQuestionStatus: newTestQuestionStatus,
    });
  }
...
const updateTestQuestionStatus = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  res.status(200).json({
    message: 'Test question status updated',
    testQuestionStatus: updatedTestQuestionStatus,
  });
});
...
const updateQuestionChoiceForTestQuestionStatus = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    res.status(200).json({
      message: 'TestQuestionStatus choices updated',
      testQuestionStatus: updatedTestQuestionStatus,
    });
  }
);
